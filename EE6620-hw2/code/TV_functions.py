...
def TVL2(img_in, k_in, max_iter, lamb_da, to_linear, gamma):
    ...
    # model the problem by proximal
    prob = Problem(sum_squares(conv(K, TVL2_result, dims=2) - img) + lamb_da * group_norm1(grad(TVL2_result, dims=2), [3]) + nonneg(TVL2_result))
...
def TVpoisson(img_in, k_in, max_iter, lamb_da, to_linear, gamma):
    ...
    # model the problem by proximal
    prob = Problem(poisson_norm(conv(K, TVpoisson_result, dims=2), bp=img) + lamb_da * group_norm1(grad(TVpoisson_result, dims=2), [3]) + nonneg(TVpoisson_result))
